...
_TreemapD3.prototype.getView = function () {
    ...
    if (!CssClassDefined("node")) {
        $('head').append('<style type="text/css">.node {border: solid 1px white;font: 10px sans-serif;line-height: 12px;overflow: hidden;position: absolute;text-indent: 2px;}.tooltip {position: absolute;border:1px solid;margin:10px;padding: 6px;opacity: 0.9;z-index: 1000;}</style>');
    }
   ...
   .style("background-color", "#ffffff");
    ...
    div.datum(me.testdata()).selectAll(".node")
     ...
     .on("mouseover", function(d) {
         tooltip.html(d.name +" Value " + d.size + " Gewinn:" +d.pkm1);
